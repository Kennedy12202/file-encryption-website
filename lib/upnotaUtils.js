...
// Store AES key in secure local storage
...
export async function createShareLink(file) {
    try {
        ...
        const expires_at = new Date(Date.now() + 5 * 1000); 
        ...
        // Store the key for this share in secureLocalStorage
        const shareKeys = JSON.parse(secureLocalStorage.getItem('shareKeys') || '{}');
        shareKeys[data.id] = await getStoredAESKey(file.cid);
